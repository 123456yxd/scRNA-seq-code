...
with open(ID + '_selected_cumulative_frequency_table.txt') as fin:
	for line in fin:
        	name = line.strip().split('\t')[1]
                cell_barcode = line.strip().split('\t')[1]
		name = name.replace(ID,"R2")
	    	os.system('samtools view -h ' + sample + "_FC_directional_grouped_2/" + name + '_group_FC.bam > ' + sample + "_FC_directional_grouped_2/" + name + '_group_FC.sam')
	    	sam = open(sample + '_FC_directional_grouped_2/' + name + '_group_FC.sam')
	    	...
	    	for line in sam:
        		...
        		if line.split('\t')[1] == '4':
            			...
            			if 'XT:Z' in line:
                			...
                			gene = gene.split('\t')[0]
                			if ('rRNA' not in gene) & (num_matches > 10):
                    				gene = 'ambiguous'
            			else:
                			gene = 'no_feature'
