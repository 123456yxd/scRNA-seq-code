...
for line in table:
    name = line.split('\t')[1]
    R1_file_name = sample + '_selected_cells/' + sample + '_R1' + name[name.find('_bc1_'):len(name)] + '.fastq.gz'
    R1_file_name = R1_file_name.replace(' ' , '')
    R2_file_name = sample + '_selected_cells/' + sample + '_R2' + name[name.find('_bc1_'):len(name)] + '.fastq.gz'
